...
class UserdetailController extends Controller
{
    ...
    public function check(Request $request)
    {
        ...
        // Assuming you want to find a user that matches both name and email
        $details = Userdetail::where('name', $request->name)
            ->where('email', $request->email)
            ->get();

        // Check if the query returned any results
        if ($details->isEmpty()) {
            return back()->with('error', 'No user found with the given details.');
        }
